...
class HelpCommand(BaseCommand):
    ...
    async def run(self, args: Namespace, update: telegram.Update, context: _common.ExtendedContext) -> None:
        ...
        if args.command:
            msg = help.get_help_for_command(args.command)
        else:
            try:
                ...
                msg = await help.get_help_usage(self.usage, context.application.client, None)
                await util.safe_call(
                    lambda: update.effective_message.reply_markdown(msg),
                    lambda: update.effective_message.reply_text(msg)
                )
                raise
            msg = await help.get_help_usage(self.usage, context.application.client, user)
